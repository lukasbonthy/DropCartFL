...
export function BookingForm(){
 ...
 >
  ...
  <p className="text-sm leading-relaxed text-muted-foreground">Keep this reference for your records. No payment has been taken and no helper has been dispatched.</p><div className="mt-5 flex flex-col gap-2 sm:flex-row"><a className="submit-button inline-flex min-h-12 items-center justify-center rounded-xl px-5 no-underline" href={"/track?ref="+encodeURIComponent(receipt.reference)}>Track driver & message <ArrowRight className="ml-2"/></a></div>
     ...
     <AnimatePresence mode="popLayout" custom={direction} initial={false}>
      <motion.div key={`step-${step}`} layout="position" className="booking-step-panel space-y-4" custom={direction} variants={stepVariants} initial="enter" animate="center" exit="exit" transition={reduceMotion?{duration:0}:quickSpring}>
       ...
       {step===2&&<>
        ...
        <label htmlFor="consent" className="flex items-start gap-3 text-sm leading-relaxed"><Checkbox id="consent" checked={draft.consent} onCheckedChange={v=>field("consent",v===true)} className="mt-0.5 size-5 shrink-0 rounded-md" aria-invalid={!!errors.consent}/><span>You can text or call me about this unload. I understand my booking needs team confirmation.</span></label>{errorFor("consent")}
       </>}
      </motion.div>
     </AnimatePresence>
